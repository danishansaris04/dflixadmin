...
// ---------- Interstitial ad timer (every 3 min while browsing) ----------
// Fires only while: user is logged in (main screen visible, not login
// screen), app is in the foreground (tab/page visible), and it's been
// at least AD_INTERVAL_MS since the last ad. This calls a SEPARATE
// throwaway native Activity (InterstitialAdActivity) for the ad itself,
// so it can never touch this WebView's login/session state.
var AD_INTERVAL_MS = 2 * 60 * 1000 // 3 minutes
var adTimerHandle = null;

function startAdTimer() {
  if (adTimerHandle) return; // already running
  adTimerHandle = setInterval(function () {
    var onLoginScreen = loginScreen && !loginScreen.classList.contains("hidden");
    if (document.hidden || onLoginScreen) return; // app backgrounded or not logged in
    try {
      Android.showInterstitialAd();
    } catch (e) {
      // Older APK without this bridge method yet - fail silently.
    }
  }, AD_INTERVAL_MS);
}
